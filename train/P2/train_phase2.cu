...
#include "../../phase2_gpu_basic/Include/kernels/backward_kernels.h"
#include "../../phase2_gpu_basic/Include/utils/mse_loss.h"
...
int main() {
    ...
    for (int epoch = 1; epoch <= NUM_EPOCHS; epoch++) {
        ...
        int update_interval = total_batches / 20;  
        ...
        while (loader.has_next()) {
            ...
            for (int b = 0; b < BATCH_SIZE; b++) {
                ...
                float forward_time_ms = chrono::duration<float, milli>(forward_end - forward_start).count();
                epoch_forward_time += forward_time_ms;
                ...
                float backward_time_ms = chrono::duration<float, milli>(backward_end - backward_start).count();
                epoch_forward_time += forward_time_ms;
            ...
            if (num_batches % update_interval == 0 || num_batches == total_batches) {
                ...
                int progress_pct = ((num_batches * 20) / total_batches) * 5;  
        ...
        if (avg_loss < best_loss) {
            ...
            float avg_forward_time = epoch_forward_time / (num_batches * BATCH_SIZE);
        float avg_backward_time = epoch_backward_time / (num_batches * BATCH_SIZE);
        cout << "Epoch " << epoch << " complete - Avg Loss: " << fixed << setprecision(6) << avg_loss 
    ...
    if (sample_images_success) {
        ...
        cout << "  - logs/sample_original_phase2.ppm" << endl;
        cout << "  - logs/sample_reconstructed_phase2.ppm" << endl;
    } else {
