...
int main() {
    ...
    for (int epoch = 1; epoch <= NUM_EPOCHS; epoch++) {
        ...
        if (weight_file.is_open()) {
            ...
            if (avg_loss == best_loss) {
                ofstream best_file("weights/phase3_v2_best.bin", ios::binary);
                if (best_file.is_open()) {
                    ...
                    best_file.close();
                    cout << "[INFO] ⭐ Saved BEST model to weights/phase3_v2_best.bin (Loss: " << fixed << setprecision(6) << best_loss << ")" << endl;
                } else {
                    cerr << "[WARNING] Could not save best model to weights/phase3_v2_best.bin" << endl;
                }
            }
        } else {
