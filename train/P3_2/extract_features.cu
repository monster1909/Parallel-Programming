...
int main(int argc, char** argv) {
    ...
    const int LATENT_SIZE = LATENT_C * LATENT_H * LATENT_W;  // 8192
    const int BATCH_SIZE = 100;  // Process 100 images at a time
    
    // Allocate weight vectors (only need encoder weights)
    ...
    float *d_pool2_out = (float*)gpu_malloc(LATENT_SIZE * sizeof(float));  // Bottleneck
    
    // Host buffer for latent features
    ...
    // Process training data (50k images)
    {
        ...
        while (train_loader.has_next()) {
            ...
            for (int b = 0; b < current_batch_size; b++) {
                ...
                // Conv2 + ReLU + MaxPool → Bottleneck (128×8×8)
                ...
                // Write label (1 byte) + features (8192 floats)
