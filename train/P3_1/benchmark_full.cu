...
void conv2d_backward_weights_gemm(const float* grad_output, const float* input, float* grad_weights, float* col_buffer, int H_in, int W_in, int C_in, int H_out, int W_out, int C_out, int ksize, int pad, int stride);
void conv2d_backward_input_gemm(const float* grad_output, const float* weights, float* grad_input, float* col_buffer, int H_in, int W_in, int C_in, int H_out, int W_out, int C_out, int ksize, int pad, int stride);
...
int main() {
    cout << "============================================" << endl;
    cout << " P3_1 Timing Benchmark (Im2Col + GEMM)" << endl;
    cout << "============================================" << endl << endl;
    ...
    cout << "--- FORWARD PASS (Im2Col + GEMM) ---" << endl << endl;
    ...
    cout << "\n--- BACKWARD PASS (Im2Col + GEMM) ---" << endl << endl;
    auto total_bwd_start = high_resolution_clock::now();
    start = high_resolution_clock::now();
    conv2d_backward_weights_gemm(d_grad_output, d_ups2_out, d_w_final, d_col_buffer, H, W, 256, H, W, C, 3, 1, 1);
    conv2d_backward_input_gemm(d_grad_output, d_w_final, d_grad_ups2_out, d_col_buffer, H, W, 256, H, W, C, 3, 1, 1);
    cudaDeviceSynchronize();
    end = high_resolution_clock::now();
    ...
    start = high_resolution_clock::now();
    conv2d_backward_weights_gemm(d_grad_dec2_out, d_ups1_out, d_w_dec2, d_col_buffer, H/2, W/2, 128, H/2, W/2, 256, 3, 1, 1);
    conv2d_backward_input_gemm(d_grad_dec2_out, d_w_dec2, d_grad_ups1_out, d_col_buffer, H/2, W/2, 128, H/2, W/2, 256, 3, 1, 1);
    cudaDeviceSynchronize();
    end = high_resolution_clock::now();
    ...
    start = high_resolution_clock::now();
    conv2d_backward_weights_gemm(d_grad_dec1_out, d_pool2_out, d_w_dec1, d_col_buffer, H/4, W/4, 128, H/4, W/4, 128, 3, 1, 1);
    conv2d_backward_input_gemm(d_grad_dec1_out, d_w_dec1, d_grad_pool2_out, d_col_buffer, H/4, W/4, 128, H/4, W/4, 128, 3, 1, 1);
    cudaDeviceSynchronize();
    end = high_resolution_clock::now();
    ...
    start = high_resolution_clock::now();
    conv2d_backward_weights_gemm(d_grad_conv2_out, d_pool1_out, d_w_conv2, d_col_buffer, H/2, W/2, 256, H/2, W/2, 128, 3, 1, 1);
    conv2d_backward_input_gemm(d_grad_conv2_out, d_w_conv2, d_grad_pool1_out, d_col_buffer, H/2, W/2, 256, H/2, W/2, 128, 3, 1, 1);
    cudaDeviceSynchronize();
    end = high_resolution_clock::now();
    ...
    start = high_resolution_clock::now();
    conv2d_backward_weights_gemm(d_grad_conv1_out, d_input, d_w_conv1, d_col_buffer, H, W, C, H, W, 256, 3, 1, 1);
    conv2d_backward_input_gemm(d_grad_conv1_out, d_w_conv1, d_input, d_col_buffer, H, W, C, H, W, 256, 3, 1, 1);
    cudaDeviceSynchronize();
    end = high_resolution_clock::now();
